...
export async function runPublish(projectRoot: string, outputDir?: string): Promise<void> {
  ...
  if (p.isCancel(author)) { p.cancel('Cancelled'); return; }
  ...
  if (p.isCancel(version)) { p.cancel('Cancelled'); return; }
  ...
  if (p.isCancel(keywordsRaw)) { p.cancel('Cancelled'); return; }
