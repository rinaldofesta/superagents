...
`;
  ...
  private async executePrompt(
    ...
    } else if (context.authMethod === 'oauth') {
      // For OAuth, use the access token
      if (!context.accessToken) {
        throw new Error('Access token is required for OAuth auth method');
      }

      const anthropic = new Anthropic({ apiKey: context.accessToken });
      const maxTokens = MAX_TOKENS_BY_TYPE[generationType] || DEFAULT_MAX_TOKENS;

      log.debug(`Calling API with OAuth token, model: ${model}, max_tokens: ${maxTokens}`);
