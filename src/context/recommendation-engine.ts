...
const REQUIREMENT_AGENTS: Record<
  ProjectRequirement,
  { agents: string[]; skills: string[]; reason: string }
> = {
  auth: {
...
export class RecommendationEngine {
  ...
  private static readonly TECH_KEYWORDS: Record<
    string,
  ...
  recommend(goal: ProjectGoal, codebase: CodebaseAnalysis): Recommendations {
    ...
    for (const agent of agents) {
      const specificReason = this.buildProjectSpecificReason(
        agent.name,
        codebase,
        goal,
      );
  ...
  private buildProjectSpecificReason(
    ...
    const { detectedPatterns, dependencies, totalFiles, testCommand } =
      codebase;
    ...
    const componentPattern = detectedPatterns.find(
      (p) => p.type === "components",
    );
    ...
    switch (agentName) {
      ...
      case "devops-specialist": {
        if (
          depNames.has("docker") ||
          codebase.detectedPatterns.some((p) =>
